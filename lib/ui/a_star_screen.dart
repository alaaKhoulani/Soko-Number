...
class AStarScreen extends StatefulWidget {
  ...
  AStarScreen({super.key, required this.currentLevel, required this.level_ind});
...
class _AStarScreenState extends State<AStarScreen> {
  ...
  void initState() {
    ...
    timer = Timer.periodic(Duration(milliseconds: 500), (tim) {
      print(index);
      // stopwatch = Logic.stopwatchAStar;
      // stopwatch.stop();
      visited = Logic.visitedNodeAStar;
      if (index - 1 >= 0) {
        setState(() {
          done = true;
          index--;
  ...
  playAStar() {
    path.clear();
    path = logic.a_star(widget.currentLevel);
                ...
                children: [
                  ...
                  SizedBox(
                    height: 70,
                      ...
                      ? detailsContainer(
                          child: Text(
                              "Time of Excute: ${Logic.stopwatchAStar.elapsed}",
                              style: detailsStyle))
                      : Container(),
                  done
                      ? detailsContainer(
