...
class DFSScreen extends StatefulWidget {
  ...
  DFSScreen({super.key, required this.currentLevel, required this.level_ind});
...
class _DFSScreenState extends State<DFSScreen> {
  ...
  void initState() {
    ...
    timer = Timer.periodic(Duration(milliseconds: 500), (tim) {
      print(index);
      // stopwatch = Logic.stopwatchDfs;
      // stopwatch.stop();
      visited = Logic.visitedNodesDFS;
      if (index - 1 >= 0) {
        setState(() {
          done = true;
          index--;
  ...
  playDFs() {
    path.clear();
    path = logic.dfs(widget.currentLevel);
                ...
                children: [
                  ...
                  SizedBox(
                    height: 70,
                      ...
                      ? detailsContainer(
                          child: Text(
                              "Time of Excute: ${Logic.stopwatchDfs.elapsed}",
                              style: detailsStyle))
                      : Container(),
                  done
                      ? detailsContainer(
